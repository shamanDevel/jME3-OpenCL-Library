...
public class HeadlessContext {
	...
	public boolean createOpenCLContext(boolean userSelection) {
		...
		//create context
		try {
			ArrayList<CLDevice> deviceList = new ArrayList<>(Arrays.asList(clDevice.getDevice()));
			CLContext c = CLContext.create(clPlatform.getPlatform(), deviceList, Utils.errorBuffer);
			ArrayList<LwjglDevice> deviceList2 = new ArrayList<>(Arrays.asList(clDevice));
			clContext = new com.jme3.opencl.lwjgl.LwjglContext(c, deviceList2);
		} catch (Exception ex) {
